...
void Uart_ReceiveFrame(u8 byte, UART_RCVFRAME *pRcvFrame)
{
    switch(pRcvFrame->state)
    {
        ...
        case UART_STAT_DATA:
            ...
            if(pRcvFrame->buffer[2] == 0)				//丢失字节，猜测此处导致溢出
            {
    ...
    pRcvFrame->idleTime = 0;									//持续接收、时间更新
}
*/
...
void Uart_ReceiveFrame( u8 byte, UART_RCVFRAME *pRcvFrame)
{
    switch(pRcvFrame->state)
    {
        ...
        case UART_STAT_DATA:
			...
			if(pRcvFrame->index >= UART_BUFFER_MAX_LEN || pRcvFrame->length >= UART_BUFFER_MAX_LEN)
			{
				pRcvFrame->state = UART_STAT_END;
				pRcvFrame->index = UART_BUFFER_MAX_LEN;
			}
				break;
    }
